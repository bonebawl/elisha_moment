...
function Clawk_timer() {
  ...
  const handleAdd = () => {
    ...
    if (taskTime < currentTime || !inputTime.match(/^([01]\d|2[0-3]):([0-5]\d)$/)) {
        //add sound guro diri, popup?? nah maybe
      setInputTime('00:00');
    } else {
      if (inputString.trim()) {
  ...
  return (
    <div className="App">
      <h1 className="text-2xl font-bold mb-4">Clock and To-Do List</h1>
      <div className="clock-container flex justify-center items-center mb-8">
        <div className="clock bg-black text-white rounded-full flex items-center justify-center">
